    ...
    <div style="min-height: 600px" >
        <el-card shadow="never" style="min-height: 400px" v-if="$page.post.edges[0].node.id">
            ...
            <div style="font-size: 0.9rem;line-height: 1.5;color: #606c71;">
                发布 {{$page.post.edges[0].node.created_at}}
                <br> 更新{{$page.post.edges[0].node.updateTime}}
            </div>
            <div style="font-size: 1.1rem;line-height: 1.5;color: #303133;border-bottom: 1px solid #E4E7ED;padding: 5px 0px 5px 0px">
                <pre style="font-family: '微软雅黑'">{{$page.post.edges[0].node.description}}</pre>
            </div>
            <div  class="markdown-body" style="padding-top: 20px"></div>
        </el-card>
...
<page-query>
    query{
        post:allStrapiPosts(order: DESC,limit:1){
            edges{
                node{
                  id
                  title
